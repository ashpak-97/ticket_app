 ...
 *  @version    1.0.4 (last revision: October 12, 2006)
...
class imageTransform
{
    ...
    function CreateThumbnail($src_file, $dest_file, $img_ht, $img_wt, $quality = 98)
    {
        //$imageTrans= new imageTransform;
        $this->jpegOutputQuality = $quality;
        $this->sourceFile = $src_file;
        $this->targetFile = $dest_file;
        $this->resizeToWidth = $img_wt;
        $this->resizeToHeight = $img_ht;
        $this->resize();
    }
}

//uploaded temp filename, Appropriate filename, DOCROOT, Target Width, Target Height
function ThumbnailImage($TMPFILENAME, $NEWNAME, $UPLOADPATH_DOCROOT, $IMG_W, $IMG_H, $FIXED_W = '', $FIXED_H = '')
{
    $dir = opendir($UPLOADPATH_DOCROOT);

    if (!THUMBNAIL_ALLOWED) {
        copy($UPLOADPATH_DOCROOT . $TMPFILENAME, $UPLOADPATH_DOCROOT . $NEWNAME);
    } else {
        list($image_wt, $image_ht, $tmp01, $tmp02) = getimagesize($UPLOADPATH_DOCROOT . $TMPFILENAME);

        $IMAGE_WIDTH = $image_wt;
        $IMAGE_HEIGHT = $image_ht;
        if (!empty($FIXED_W)) $IMAGE_WIDTH = $IMG_W;
        if (!empty($FIXED_H)) $IMAGE_HEIGHT = $IMG_H;

        //echo $UPLOADPATH_DOCROOT.$TMPFILENAME.",". $UPLOADPATH_DOCROOT.$NEWNAME.",". $IMAGE_WIDTH.",". $IMAGE_HEIGHT;

        if ($image_wt < $IMG_W && $image_ht < $IMG_H) {
            copy($UPLOADPATH_DOCROOT . $TMPFILENAME, $UPLOADPATH_DOCROOT . $NEWNAME); //, $IMAGE_WIDTH, $IMAGE_HEIGHT);
        } else {
            if ($image_wt >= $image_ht) //landscape view
            {
                if ((($image_ht * $IMG_W) / $image_wt) <= $IMG_H) {
                    $ht = -1;
                    $wt = $IMG_W;
                } else {
                    $ht = $IMG_H;
            ...
            {
                if ((($image_wt * $IMG_H) / $image_ht) <= $IMG_W) {
                    $ht = $IMG_H;
                    $wt = -1;
                } else {
                    $ht = -1;
            ...
            if (empty($IMAGE_WIDTH)) $IMAGE_WIDTH = $wt;
            if (empty($IMAGE_HEIGHT)) $IMAGE_HEIGHT = $ht;

            //CREATION OF THUMBNAIL		
            $imageTrans = new imageTransform;
            $imageTrans->CreateThumbnail($UPLOADPATH_DOCROOT . $TMPFILENAME, $UPLOADPATH_DOCROOT . $NEWNAME, $IMAGE_HEIGHT, $IMAGE_WIDTH);
        }
    }
